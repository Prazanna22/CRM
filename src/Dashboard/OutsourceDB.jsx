  ...
  const startCall = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        // ✅ Fetch updated data after slight delay
        setTimeout(() => {
          fetchData();
    ...
    <div className="px-4 pt-5 ibm">
      ...
      <div className="flex justify-between gap-10">
        ...
        <select value={filter} onChange={(e) => setFilter(e.target.value)} className="cursor-pointer border rounded border-white py-1 px-4 my-2">
          ...
          <option value="today" className="cursor-pointer bg-black">Today</option>
        </select>

        <div className="flex flex-col md:flex-row md:gap-5">
          <button
            className="bg-red-600 text-white px-4 py-1 text-md rounded cursor-pointer hover:bg-red-700 my-2"
            onClick={stopCall}
          >
            Stop Call
          </button>
            ...
            <thead className="bg-gray-200 sticky top-0 z-10">
              <tr>
                {[
                  ...
                  <th
                    key={index}
                    className="px-3 py-3 text-left text-xs font-medium text-gray-900 uppercase tracking-wider whitespace-nowrap"
                  >
                    {header}
                  </th>
                ))}
